...
function BookDetails({ book }) {
    ...
    const bookDetails = useSelector((store) => {
        return { book: store.books.books.find((b) => b.id === parseInt(id)), genre: store.books.categories.find(cat => cat.id === store.books.books.find((b) => b.id === parseInt(id))?.categoryId)?.name || 'Unknown' };
    });
